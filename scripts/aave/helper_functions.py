...
def get_token(token_name):
    token = interface.IERC20Detailed(config["networks"][network.show_active()][token_name])
    return token
...
def get_variable_token(token_name):
    ...
    token = interface.IVariableDebtToken(config["networks"][network.show_active()][token_name])
    return token

def get_stable_token(token_name):
    # stableDebtAAVE, stableDebtDAI, stableDebtLINK, stableDebtSUSD etc...
    token = interface.IStableDebtToken(config["networks"][network.show_active()][token_name])
    return token
