...
def main():
    ...
    registry = Registry.at(
        get_address("Vault Registry", default="v2.registry.ymsaki.eth")
    )
    ...
    if use_proxy:
        gov_default = (
            "0xFE948CB2122FDD87bAf43dCe8aFa254B1242c199"  # strategist msig, no ENS
        )
    else:
        gov_default = "0xFE948CB2122FDD87bAf43dCe8aFa254B1242c199"
    gov = get_address("Yearn Governance", default=gov_default)

    rewards = get_address("Rewards contract", default="treasury.ymsaki.eth")
    guardian = gov
    if use_proxy == False:
        guardian = get_address("Vault Guardian", default="dev.ymsaki.eth")
    management = get_address("Vault Management", default="ymsaki.eth")
