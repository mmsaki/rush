...
contract yWETH is ReentrancyGuard, rToken {
    using Address for address payable;

    constructor(address _weth, address _registry) public rToken(_weth, _registry) {}
    ...
    function depositETH() public payable returns (uint256) {
        ...
        // NOTE: Need to use different method to deposit than `rToken`
    ...
    function withdrawETH(uint256 amount) external nonReentrant returns (uint256 withdrawn) {
        // NOTE: Need to use different method to withdraw than `rToken`
